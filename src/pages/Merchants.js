...
const loginId = Number(localStorage.getItem("id"))
  ...
  useEffect(() => {
      (async () => {
        const { data } = await axios.get(`merchant?page=${page}`);
  ...
  return (
    <Wrapper>
      <div className="pt-3 pb-2 mb-3 border-bottom">
        <Link to={"/merchants/create"} className="btn btn-sm btn-outline-secondary">
          Add Merchant
        </Link>
      </div>
                ...
                <tr key={merchant.id}>
                  ...
                  <td>
                    ...
                    <Link to={`/merchants/${merchant.id}/view`} className="btn btn-sm btn-outline-secondary">Action</Link>
                      <Link to={`/merchants/${merchant.id}/assign`} className="btn btn-sm btn-outline-secondary">Assign</Link>
                      {/* <a
